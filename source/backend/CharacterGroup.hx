 ...
 * `FlxSpriteContainer` instead of `FlxSpriteGroup`.
 =================================================================
 * # CharacterGroup, or... FlxCharacterGroup?
 * Nuh uh, just CharacterGroup, i made this class just to acces
   `Character's` methods and props easily.

   - AssmanBruh
 */
typedef CharacterGroup = FlxTypedSpriteGroup<Character>;

/**
 * Dupp the FlxSpriteGroup class to allow for typed groups of `Character` objects.
 * Editted by @AssmanBruh!
 */
class FlxTypedSpriteGroup<T:Character> extends Character
{
	/**
	...
	function findMinXHelper()
	{
		...
		for (member in group.members)
		{
			...
			if (member.flixelType == SPRITEGROUP)
				minX = (cast member:CharacterGroup).findMinX();
			else
				minX = member.x;
	...
	function findMaxXHelper()
	{
		...
		for (member in group.members)
		{
			...
			var maxX:Float;
			if (member.flixelType == SPRITEGROUP)
				maxX = (cast member:CharacterGroup).findMaxX();
			else
	...
	function findMinYHelper()
	{
		...
		for (member in group.members)
		{
			...
			var minY:Float;
			if (member.flixelType == SPRITEGROUP)
				minY = (cast member:CharacterGroup).findMinY();
			else
				minY = member.y;
	...
	function findMaxYHelper()
	{
		...
		for (member in group.members)
		{
			...
			var maxY:Float;
			if (member.flixelType == SPRITEGROUP)
				maxY = (cast member:CharacterGroup).findMaxY();
			else
	...
	{
		#if FLX_DEBUG
		throw "This function is not supported in CharacterGroup";
		#end
		return this;
	}
